...
test('岗位集锦提供首批四个可展示的岗位', () => {
  ...
  for (const job of data.jobs) {
    ...
    assert.ok(fs.existsSync(path.join(root, job.card)), `${job.slug} 的知识卡不存在`);
  }
});
...
test('岗位集锦提供聚合页与静态详情路由', async () => {
  ...
  assert.match(indexPage, /jobs-magazine/, '聚合页应使用杂志封面布局');
  assert.match(indexPage, /jobs-quick-notes/, '封面应使用研究摘要填充信息密度');
